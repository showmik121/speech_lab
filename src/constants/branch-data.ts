...
/**
 * Static demo data for the Branch Management module.
 * Mirrors the payload shape expected from the Django REST API later.
 */

export type BranchStatus = "Active" | "Inactive" | "Pending" | "Suspended";

export const BRANCH_STATUS_TONE: Record<BranchStatus, StatusTone> = {
  Active: "success",
  Inactive: "neutral",
  Pending: "warning",
  Suspended: "danger",
...
export type Branch = {
  ...
  code: string;
  name: string;
  manager: { name: string; phone: string; email: string; since: string; managerId: string };
  phone: string;
  email: string;
  city: string;
  division: string;
...
export const BRANCH_STATUSES: BranchStatus[] = ["Active", "Inactive", "Pending", "Suspended"];

export const MANAGERS = BRANCHES.map((branch) => branch.manager.name);
...
export const BRANCH_KPIS: Kpi[] = [
  ...
  },
  {
    ...
    value: new Intl.NumberFormat("en-IN").format(
      BRANCHES.reduce((sum, b) => sum + b.patients, 0),
    ),
    icon: Users,
    tone: "primary",
  ...
  },
  {
    ...
    value: `৳ ${new Intl.NumberFormat("en-IN").format(
      BRANCHES.reduce((sum, b) => sum + b.monthlyRevenue, 0),
    )}`,
    icon: Banknote,
    tone: "success",
...
export const BRANCH_ACTIVITIES: BranchActivity[] = [
  {
    id: "b1",
...
export type BranchRevenuePoint = { month: string; revenue: number; collection: number };

export const BRANCH_REVENUE_TREND: BranchRevenuePoint[] = [
  { month: "Feb", revenue: 128000, collection: 118400 },
  { month: "Mar", revenue: 139500, collection: 127800 },
  { month: "Apr", revenue: 146200, collection: 138100 },
  { month: "May", revenue: 158900, collection: 147600 },
  { month: "Jun", revenue: 171300, collection: 160200 },
  { month: "Jul", revenue: 184000, collection: 172500 },
];
...
export function findBranchByManagerId(managerId: string) {
  return BRANCHES.find(
    (branch) => branch.manager.managerId.toLowerCase() === managerId.trim().toLowerCase(),
  );
}
