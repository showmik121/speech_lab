import { AlertTriangle, Boxes, Layers, PackageCheck, PackageX, ShoppingCart, TrendingUp, Wallet } from "lucide-react";
...
export const MATERIALS_TODAY_ISO = "2026-07-31";

export type MaterialStatus = "Available" | "Low Stock" | "Out of Stock" | "Inactive";
...
export const MATERIAL_STATUS_TONE: Record<MaterialStatus, StatusTone> = {
  Available: "success",
  "Low Stock": "warning",
  "Out of Stock": "danger",
  Inactive: "neutral",
...
export const MATERIAL_CATEGORY_NAMES = MATERIAL_CATEGORIES.map(
  (category) => category.name,
) as MaterialCategoryName[];

export const MATERIAL_UNITS = ["Piece", "Set", "Box", "Pack", "Dozen"] as const;
...
export const MATERIALS: Material[] = [
  {
    ...
    brand: "Shikkha Tools",
    description:
      "144-card Bangla articulation deck covering initial, medial and final phoneme positions.",
    unit: "Set",
    purchasePrice: 620,
    sellingPrice: 950,
    quantity: 48,
...
export const SALE_PAYMENT_STATUSES: PaymentStatus[] = ["Paid", "Partial", "Due", "Refunded"];

export const SALE_STATUS_TONE: Record<PaymentStatus, StatusTone> = {
  Paid: "success",
  Partial: "warning",
  Due: "danger",
  Refunded: "neutral",
...
export const PAYMENT_METHODS = ["Cash", "Card", "Mobile Banking", "Bank"] as const;
...
function totalOf(quantity: number, unitPrice: number, discount: number) {
...
export function salesForMaterial(materialId: string) {
...
export type MovementType = "Stock In" | "Stock Out" | "Adjustment" | "Sale" | "Damaged";
...
export const MOVEMENT_TONE: Record<MovementType, StatusTone> = {
  "Stock In": "success",
  "Stock Out": "warning",
  Adjustment: "info",
  Sale: "info",
...
export const MOVEMENT_ICON: Record<MovementType, "in" | "out" | "adjust" | "sale" | "damaged"> = {
  "Stock In": "in",
  "Stock Out": "out",
  Adjustment: "adjust",
  Sale: "sale",
...
export function purchasesForMaterial(materialId: string) {
...
export type LowStockPriority = "Critical" | "High" | "Medium";

export const LOW_STOCK_PRIORITY_TONE: Record<LowStockPriority, StatusTone> = {
  Critical: "danger",
  High: "warning",
  Medium: "info",
};
...
export const LOW_STOCK_ALERTS: LowStockAlert[] = MATERIALS.filter(
  (material) => material.status === "Out of Stock" || material.status === "Low Stock",
).map((material) => {
  const required = Math.max(material.minStock * 2 - material.quantity, material.minStock);
  const priority: LowStockPriority =
    material.quantity === 0 ? "Critical" : material.quantity < material.minStock / 2 ? "High" : "Medium";
  return {
    id: `low-${material.id}`,
...
const AVAILABLE_STOCK = MATERIALS.reduce((sum, material) => sum + material.quantity, 0);
const LOW_STOCK_COUNT = MATERIALS.filter((material) => material.status === "Low Stock").length;
const OUT_OF_STOCK_COUNT = MATERIALS.filter(
  (material) => material.status === "Out of Stock",
).length;
const TODAYS_SALES = SALES.filter((sale) => sale.date === MATERIALS_TODAY_ISO).reduce(
  (sum, sale) => sum + sale.total,
  0,
);
const MONTHLY_SALES = SALES.filter((sale) => sale.date.startsWith("2026-07")).reduce(
  (sum, sale) => sum + sale.total,
  0,
);
const INVENTORY_VALUE = MATERIALS.reduce(
  (sum, material) => sum + material.quantity * material.purchasePrice,
  0,
);

export const MATERIAL_KPIS: Kpi[] = [
  {
    id: "total-materials",
  ...
  },
  {
    ...
    icon: PackageCheck,
    tone: "success",
    delta: { value: "+82 units", direction: "up" },
  ...
  },
  {
    ...
    icon: PackageX,
    tone: "danger",
    delta: { value: "Reorder raised", direction: "flat" },
  ...
  },
  {
    ...
    icon: TrendingUp,
    tone: "success",
    delta: { value: "+8.6% vs June", direction: "up" },
  ...
  },
  {
    ...
    icon: Wallet,
    tone: "primary",
    delta: { value: "At purchase cost", direction: "flat" },
