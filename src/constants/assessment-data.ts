...
/**
 * Static demo data for the Assessment Management module.
 * Shape mirrors the intended Django REST API payloads — no persistence yet.
 */
...
export const ASSESSMENT_STATUS_TONE: Record<AssessmentStatus, StatusTone> = {
  Pending: "warning",
  Completed: "info",
  Approved: "success",
  "Follow-up": "warning",
...
export const SEVERITY_TONE: Record<AssessmentSeverity, StatusTone> = {
  Mild: "success",
  Moderate: "warning",
  Severe: "danger",
};
...
export type EvaluationSkill = {
  key: string;
  label: string;
  /** 1 (significant concern) — 5 (age appropriate) */
  score: number;
  note: string;
};
...
export const THERAPY_RECOMMENDATIONS = [
  "Speech Therapy",
  "Language Therapy",
...
export const SEVERITY_OPTIONS: AssessmentSeverity[] = ["Mild", "Moderate", "Severe"];
...
export const PACKAGE_SUGGESTIONS = ["Daily", "Session Wise", "Monthly"] as const;
...
export const ASSESSMENT_DATE_RANGES = [
...
export const SCORE_TONE: Record<number, StatusTone> = {
  1: "danger",
  2: "danger",
  3: "warning",
  4: "info",
...
function evaluation(scores: number[], notes: string[] = []): EvaluationSkill[] {
...
export const ASSESSMENTS: Assessment[] = [
  {
    ...
    observationNotes:
      "Cooperative throughout the session. Responded to name inconsistently. Preferred play-based tasks over structured picture naming.",
    medicalSummary:
      "Full term birth, no NICU stay. Hearing screening normal. No prior therapy exposure.",
    evaluation: evaluation(
      [2, 2, 4, 3, 4, 3, 4, 3, 4, 3, 4],
      [
    ...
    diagnosis: {
      ...
      severity: "Moderate",
      summary:
        "Expressive language is roughly 18 months behind chronological age with multiple phonological process errors. Receptive language is comparatively stronger.",
    },
    recommendation: {
      ...
      homeInstructions:
        "Daily 15-minute picture book naming, model-and-expand technique, avoid answering on the child's behalf.",
    },
    treatmentPlan: {
      package: "Monthly",
  ...
  },
  {
    ...
    observationNotes:
      "Explored the room independently. Strong reaction to sudden sounds. Engaged briefly with cause-and-effect toys.",
    medicalSummary: "Delivered at 36 weeks. Sat at 9 months, walked at 17 months.",
    evaluation: evaluation([1, 1, 2, 2, 1, 3, 3, 3, 2, 1, 3]),
    diagnosis: {
      primary: "Autism Spectrum Disorder",
      secondary: "Language Delay",
      severity: "Severe",
      summary:
        "Pervasive social communication differences with sensory seeking behaviour. Formal ADOS-style scoring pending second visit.",
    },
    recommendation: {
      ...
      homeInstructions:
        "Structured daily routine with visual schedule, sensory diet twice a day, reduce screen exposure.",
    },
    treatmentPlan: {
      package: "Monthly",
  ...
  },
  {
    ...
    observationNotes:
      "Confident in conversation, dysfluency increases under time pressure. Uses easy-onset strategy when prompted.",
    medicalSummary: "No significant medical history. Family history of stammering (paternal uncle).",
    evaluation: evaluation([4, 4, 5, 4, 5, 4, 4, 2, 5, 4, 5]),
    diagnosis: {
      primary: "Stammering",
      secondary: "None",
      severity: "Mild",
      summary:
        "Dysfluency frequency dropped from 14% to 6% of spoken syllables since the last review. Secondary behaviours reduced.",
    },
    recommendation: {
      therapy: "Speech Therapy",
      sessionsPerWeek: 2,
      ...
      homeInstructions:
        "Daily 10-minute slow-rate reading aloud, weekly recorded speech sample for review.",
    },
    treatmentPlan: {
  ...
  },
  {
    ...
    observationNotes:
      "Quiet and observant. Warmed up after 15 minutes and named familiar objects on request.",
    medicalSummary: "Recurrent ear infections between 12 and 24 months, now resolved.",
    evaluation: evaluation([3, 2, 4, 4, 4, 4, 4, 4, 4, 2, 4]),
    diagnosis: {
      primary: "Language Delay",
      secondary: "Speech Delay",
      severity: "Mild",
      summary:
        "Mild expressive delay likely linked to earlier conductive hearing fluctuation. Comprehension within normal limits.",
    },
    recommendation: {
  ...
  },
  {
    ...
    observationNotes:
      "Highly motivated, imitates target sounds accurately with visual cues and a mirror.",
    medicalSummary: "Unremarkable medical history. Normal hearing evaluation in May 2026.",
    evaluation: evaluation([4, 4, 5, 4, 5, 3, 4, 4, 5, 4, 5]),
    diagnosis: {
      primary: "Articulation Disorder",
      secondary: "None",
      severity: "Mild",
      summary:
        "Isolated articulation errors on three phonemes with stimulability at word level. Prognosis excellent.",
    },
    recommendation: {
      therapy: "Speech Therapy",
      sessionsPerWeek: 2,
  ...
  },
  {
    ...
    observationNotes:
      "Improved pencil grip. Still avoids bilateral coordination tasks without encouragement.",
    medicalSummary: "Born at 34 weeks, low birth weight. Mild hypotonia noted at 2 years.",
    evaluation: evaluation([4, 4, 4, 3, 5, 4, 5, 5, 4, 4, 2]),
    diagnosis: {
      primary: "Global Developmental Delay",
      secondary: "None",
      severity: "Moderate",
      summary:
        "Fine motor skills tracking approximately 18 months behind peers; language and cognition age appropriate.",
    },
    recommendation: {
      ...
      homeInstructions:
        "Daily dough and peg-board activity, dressing practice before school, avoid adult over-assistance.",
    },
    treatmentPlan: {
      package: "Monthly",
      estimatedSessions: 72,
  ...
  },
  {
    ...
    evaluation: evaluation([4, 4, 4, 4, 5, 4, 2, 4, 5, 4, 5]),
    diagnosis: {
  ...
  },
  {
    ...
    observationNotes:
      "Needed frequent redirection. Performance improved markedly in a distraction-free setup.",
    medicalSummary: "No birth complications. Sleep routine irregular per guardian report.",
    evaluation: evaluation([4, 3, 2, 1, 3, 4, 4, 4, 3, 3, 4]),
    diagnosis: {
      primary: "ADHD",
      secondary: "Language Delay",
      severity: "Moderate",
      summary:
        "Attention regulation is the primary barrier to language performance; formal behavioural rating scales shared with guardians.",
    },
    recommendation: {
      ...
      homeInstructions:
        "Consistent bedtime routine, timer-based work chunks, immediate positive reinforcement.",
    },
    treatmentPlan: {
      package: "Daily",
  ...
  },
  {
    ...
    observationNotes:
      "Good motivation. Intelligibility drops from 85% at word level to 55% at sentence level.",
    medicalSummary: "Spastic diplegia, ambulatory with support. Regular physiotherapy ongoing.",
    evaluation: evaluation([3, 3, 5, 4, 5, 2, 3, 3, 4, 4, 1]),
    diagnosis: {
      primary: "Cerebral Palsy",
      secondary: "Articulation Disorder",
      severity: "Severe",
      summary:
        "Dysarthric speech pattern with reduced breath support. Steady gains recorded over the last two review cycles.",
    },
    recommendation: {
      therapy: "Speech Therapy",
      sessionsPerWeek: 3,
      ...
      homeInstructions:
        "Breath support exercises twice daily, slow-rate practice with family conversation partners.",
    },
    treatmentPlan: {
      package: "Monthly",
  ...
  },
  {
    ...
    observationNotes:
      "Responded to visual cues but not to sound from behind. Audiology referral raised on the spot.",
    medicalSummary: "Family history of childhood hearing loss on maternal side.",
    evaluation: evaluation([1, 2, 4, 3, 4, 4, 2, 3, 4, 3, 5]),
    diagnosis: {
      primary: "Hearing Impairment",
      secondary: "Speech Delay",
      severity: "Severe",
      summary:
        "Suspected sensorineural hearing loss. Language intervention to run alongside audiological management.",
    },
    recommendation: {
      therapy: "Language Therapy",
      sessionsPerWeek: 3,
      ...
      homeInstructions:
        "Face-to-face communication at the child's eye level, reduce background noise, daily listening routine.",
    },
    treatmentPlan: {
      package: "Monthly",
...
export const BRANCH_FILTER_OPTIONS = Array.from(
  new Set(ASSESSMENTS.map((assessment) => assessment.branch)),
).sort();

function isToday(date: string) {
  return new Date(date).toDateString() === new Date("2026-07-31").toDateString();
}

export const ASSESSMENT_KPIS: Kpi[] = [
  {
    id: "total",
  ...
  },
  {
    ...
    label: "New Patients",
    value: String(
      ASSESSMENTS.filter((item) => item.assessmentType === "Initial Assessment").length,
    ),
    icon: UserPlus,
    tone: "success",
...
export const ASSESSMENT_LIST_ICON: LucideIcon = ClipboardList;

export function findAssessment(id: string) {
  return ASSESSMENTS.find(
    (assessment) => assessment.id === id || assessment.code.toLowerCase() === id.toLowerCase(),
  );
}

export function averageScore(evaluationScores: EvaluationSkill[]) {
