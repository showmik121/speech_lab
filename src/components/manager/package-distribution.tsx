...
import { PACKAGE_DISTRIBUTION } from "@/constants/manager-dashboard-data";

const config = Object.fromEntries(
  PACKAGE_DISTRIBUTION.map((slice) => [slice.name, { label: slice.name, color: slice.token }]),
);
      ...
      <div className="relative">
        <ChartContainer config={config} className="aspect-auto h-[200px] w-full">
          <PieChart>
            ...
            >
              {PACKAGE_DISTRIBUTION.map((slice) => (
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[12px] text-muted-foreground">Packages</span>
        </div>
      </div>
