...
export type TherapyFilterState = {
  search: string;
  mode: string;
  status: string;
  program: string;
...
export const DEFAULT_THERAPY_FILTERS: TherapyFilterState = {
  search: "",
  mode: "all",
  status: "all",
  program: "all",
...
export function TherapyFilters({
  value,
  onChange,
  onRefresh,
  isRefreshing = false,
  ...
  value: TherapyFilterState;
  onChange: (next: TherapyFilterState) => void;
  onRefresh?: () => void;
  isRefreshing?: boolean;
  visibleColumns: TherapyColumnKey[];
  onToggleColumn: (key: TherapyColumnKey) => void;
}) {
  const set = (patch: Partial<TherapyFilterState>) => onChange({ ...value, ...patch });
    ...
    <div className="space-y-3 border-b border-border px-4 py-3.5 sm:px-5">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2.5">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuSeparator />
              {THERAPY_COLUMNS.filter((column) => column.hideable).map((column) => (
                <DropdownMenuCheckboxItem
                  key={column.key}
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <Select value={value.range} onValueChange={(range) => set({ range })}>
          <SelectTrigger className="h-9 w-[165px]" aria-label="Filter by session date">
            <SelectValue placeholder="Date" />
          </SelectTrigger>
          <SelectContent>
            {THERAPY_DATE_RANGES.map((range) => (
