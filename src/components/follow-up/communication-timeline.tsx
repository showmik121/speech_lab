...
import type { CommunicationEvent } from "@/constants/follow-up-data";
import type { StatusTone } from "@/components/common/status-badge";

/** Serializable icon keys are mapped to components here, in the view layer. */
const ICONS: Record<CommunicationEvent["icon"], LucideIcon> = {
  registered: UserPlus,
...
export function CommunicationTimeline({ items }: { items: CommunicationEvent[] }) {
  return (
      ...
      {items.map((item) => {
        ...
        return (
          <li key={item.id} className="relative">
            ...
            <p className="mt-1.5 text-[12px] text-muted-foreground/80">by {item.by}</p>
          </li>
        );
      })}
    </ol>
